...
class DBDashboard extends React.Component{
    ...
    componentDidMount(){
        ...
        pFirestore.collection("settings").doc("usersMapping").get().then(doc=>{
            var usersMap = {};
        ...
        if(pAuth.currentUser){
            ...
            try{
                pFirestore.collection("databases").doc(dbId).onSnapshot(snap=>{
                    ...
                    pFirestore.collection("databases").doc(this.state.dbId).collection("questions").orderBy("lastUpdated","desc").onSnapshot((questions)=>{
    ...
    tryMemberCode = () => {
        ...
        pFirestore.collection("databases").doc(this.state.dbId).update({memberCode: this.state.memberCodeTry, members: fbFieldValue.arrayUnion(pAuth.currentUser.uid)})
            .then(()=>{
                this.setState({tempDenyAccess: false});
            })    
            .catch((e)=>{
                console.log(e);
                this.setState({accessLevel: 0, displayError: true, tempDenyAccess: false}) //tempDenyAccess is false because you already denied access if it is an error.
            })
    }
    ...
    return(<div  id="dbdashboard-main">

        <div>
            <Link className="react-link arrow-button toDBListButton" to="/dblist"><span>{"<<< "}</span>List of Databases</Link>
        </div>

        {!this.state.isExistent?
            (<div id="nonexistent-db">This database either does not exist no longer exists</div>)
            :(<div>
            <div id="db-title">
            ...
            <div>
                {this.state.accessLevel<=1&&<div>
                    <h3>No Editing Access</h3>
                    <input onChange={this.changeState} name="memberCodeTry" placeholder="Member Code" value={this.state.memberCodeTry}></input>
                    <button onClick={this.tryMemberCode} className="sb">Submit</button>
                </div>}

                <div className="switch-menu" id="db-dashboard-switch-menu">
                    <button onClick={()=>{this.setState({menuOption: 0})}} className={this.state.menuOption==0?"selected":""}>
                        <div>Questions</div>
                        <span></span>
                    </button>
                    <button onClick={()=>{this.setState({menuOption: 1})}} className={this.state.menuOption==1?"selected":""}>
                        <div> Settings {"&"} Admin</div>
                        <span></span>
                    </button>
                </div>

                {this.state.menuOption==0?<div>
                    ...
                    <h3>To administer this database, you must be promoted by an existing admin and have editing access.</h3>
                </div>}


                

                {this.state.displayError&&<div className="grayed-out-background">
                    <div className="popup">
                        <div>Access Error</div>
                        <button className="cancel-button" onClick={()=>this.setState({displayError: false})}>Try Again</button>
                    </div>
